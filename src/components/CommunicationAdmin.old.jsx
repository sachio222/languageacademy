...
const CommunicationAdmin = () => {
  ...
  const handleTestEmail = async () => {
    setTestLoading(true);
    try {
      // For testing only - get API key from prompt
      const apiKey = prompt('Enter your MailerLite API key for testing:');
      if (!apiKey) {
        setTestLoading(false);
        return;
      }

      // Find and send your "test" campaign
      const listResponse = await fetch('https://connect.mailerlite.com/api/campaigns', {
        headers: { 'Authorization': `Bearer ${apiKey}` }
      });
      
      const campaigns = await listResponse.json();
      const testCampaign = campaigns.data?.find(c => c.name.toLowerCase().includes('test'));
      
      if (!testCampaign) {
        alert('❌ No campaign with "test" in name found');
        return;
      }

      if (testCampaign.status !== 'ready') {
        alert(`❌ Campaign "${testCampaign.name}" is ${testCampaign.status}. Must be "ready" to schedule. Complete it in MailerLite dashboard first.`);
        return;
      }

      // Schedule the test campaign to send now
      const response = await fetch(`https://connect.mailerlite.com/api/campaigns/${testCampaign.id}/schedule`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        alert(`✅ Test campaign "${testCampaign.name}" scheduled! Check brainpowerux@gmail.com`);
      } else {
        alert('❌ MailerLite Error: ' + JSON.stringify(result, null, 2));
      }
    } catch (error) {
            ...
            {activeTab === 'overview' && (
              <div className="overview-tab">
                ...
                <div className="test-section">
                  ...
                  >
                    {testLoading ? 'Sending...' : 'Send Test Email to brainpowerux@gmail.com'}
                  </button>
                </div>
