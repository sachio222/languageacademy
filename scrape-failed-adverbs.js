...
/**
 * Cambridge Dictionary Scraper for Failed Adverbs (Retry)
 * Re-scrapes the adverbs that previously failed with improved logic
 */

// Failed adverbs from the previous run
const failedAdverbs = [
  "toujours",
  "jamais",
  "hyper",
      ...
      for (let i = 0; i < headEntries.length; i++) {
        ...
        if (posElement) {
          ...
          // 5. If one is matching, then scrape it (more flexible matching)
            ...
            expectedPartOfSpeech === "adverb" &&
            (posText.includes("adverb") ||
              posText.includes("adv") ||
              posText.includes("adverbial"))
          ) {
            console.log(
              `    ✅ Found flexible adverb match in POS at index ${i + 1}`
            );
            foundMatch = true;
          }
        }
...
async function scrapeFailedAdverbs() {
  console.log(
    `\n🚀 Re-scraping ${failedAdverbs.length} failed adverbs with improved logic...`
  );

  const scrapedAdverbs = new Map();
  const stillFailedAdverbs = [];

  let count = 0;
  for (const word of failedAdverbs) {
    count++;
    console.log(
      `\n📝 Processing ${count}/${failedAdverbs.length}: ${word} (adverb)`
    );

    try {
      const scrapedData = await scrapeCambridgeWord(word, "adverb");

      if (scrapedData) {
        scrapedAdverbs.set(`${word}-fr`, scrapedData);
        console.log(`✅ Successfully scraped ${word}`);
      } else {
        stillFailedAdverbs.push({
          word: word,
  ...
  // Save results
  const outputPath =
    "src/data/dictionary/words/cambridge/failed-adverbs-retry.js";
  const outputContent = `/**
 * Cambridge Dictionary Scraped Failed Adverbs (Retry)
 * Generated: ${new Date().toISOString()}
 * Successfully scraped: ${scrapedAdverbs.size}/${failedAdverbs.length}
 */

export const failedAdverbsRetry = new Map([
${Array.from(scrapedAdverbs.entries())
...
export const retryStats = {
  ...
  total_failed_adverbs: ${failedAdverbs.length},
  successfully_scraped: ${scrapedAdverbs.size},
  still_failed: ${stillFailedAdverbs.length},
  still_failed_adverbs: ${JSON.stringify(stillFailedAdverbs, null, 2)}
};
`;
  ...
  console.log(`📊 Results:`);
  console.log(`  - Total failed adverbs: ${failedAdverbs.length}`);
  console.log(`  - Successfully scraped: ${scrapedAdverbs.size}`);
  console.log(`  - Still failed: ${stillFailedAdverbs.length}`);
  console.log(`  - Saved to: ${outputPath}`);

  if (stillFailedAdverbs.length > 0) {
    console.log(`\n❌ Still failed adverbs:`);
    stillFailedAdverbs.forEach((failed) => {
...
// Run the scraper
scrapeFailedAdverbs().catch(console.error);
