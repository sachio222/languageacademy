...
/**
 * Cambridge Dictionary Scraper for Interjections
 * Scrapes all interjections with proper part of speech matching
 */
      ...
      for (let i = 0; i < headEntries.length; i++) {
        ...
        if (posElement) {
          ...
          // 5. If one is matching, then scrape it (more flexible matching)
            ...
            expectedPartOfSpeech === "interjection" &&
            (posText.includes("interjection") ||
              posText.includes("exclamation") ||
              posText.includes("greeting") ||
              posText.includes("exclam"))
          ) {
            console.log(
              `    ✅ Found flexible interjection match in POS at index ${
                i + 1
              }`
            );
            foundMatch = true;
...
async function scrapeAllInterjections() {
  console.log(
    `\n🚀 Starting Cambridge scraping for ${interjections.size} interjections...`
  );

  const scrapedInterjections = new Map();
  const failedInterjections = [];

  let count = 0;
  for (const [id, interjectionData] of interjections) {
    count++;
    console.log(
      `\n📝 Processing ${count}/${interjections.size}: ${interjectionData.word} (${interjectionData.partOfSpeech})`
    );

    try {
      const scrapedData = await scrapeCambridgeWord(
        interjectionData.word,
        interjectionData.partOfSpeech
      );

      if (scrapedData) {
        scrapedInterjections.set(id, scrapedData);
        console.log(`✅ Successfully scraped ${interjectionData.word}`);
      } else {
        failedInterjections.push({
          word: interjectionData.word,
          reason: "No data returned",
        });
        console.log(
          `❌ Failed to scrape ${interjectionData.word} - no data returned`
        );
      }
    } catch (error) {
      failedInterjections.push({
      ...
      console.log(
        `❌ Failed to scrape ${interjectionData.word} - ${error.message}`
      );
    }

    // Wait random 3-5 seconds between requests
    if (count < interjections.size) {
  ...
  const outputData = {
    ...
    total_interjections: interjections.size,
    successful_scrapes: scrapedInterjections.size,
    failed_scrapes: failedInterjections.length,
    interjections: Object.fromEntries(scrapedInterjections),
    failed_interjections: failedInterjections,
  };

  const outputPath = "src/data/dictionary/words/cambridge/interjections.js";
  const outputContent = `/**
 * Cambridge Dictionary Scraped Interjections
 * Generated: ${new Date().toISOString()}
 * Successfully scraped: ${scrapedInterjections.size}/${interjections.size}
 */

export const interjectionsCambridge = new Map([
${Array.from(scrapedInterjections.entries())
...
export const scrapeStats = {
  ...
  total_interjections: ${outputData.total_interjections},
  successful_scrapes: ${outputData.successful_scrapes},
  failed_scrapes: ${outputData.failed_scrapes},
  failed_interjections: ${JSON.stringify(
    outputData.failed_interjections,
    null,
    2
  )}
};
`;
  ...
  console.log(`\n🎉 Scraping complete!`);
  console.log(`📊 Results:`);
  console.log(`  - Total interjections: ${interjections.size}`);
  console.log(`  - Successfully scraped: ${scrapedInterjections.size}`);
  console.log(`  - Failed: ${failedInterjections.length}`);
  console.log(`  - Saved to: ${outputPath}`);

  if (failedInterjections.length > 0) {
    console.log(`\n❌ Failed interjections:`);
    failedInterjections.forEach((failed) => {
...
// Run the scraper
scrapeAllInterjections().catch(console.error);
