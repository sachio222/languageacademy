/**
 * Extract Capitalized Article Phrases
 * Find phrases starting with articles (l', la, le, les, un, une) that have capital letters
...
console.log(
  "🔍 Extracting capitalized article phrases (l', la, le, les, un, une)..."
);
...
try {
  ...
  // Filter phrases that start with articles and have capital letters
  const capitalizedArticlePhrases = entries.filter(([id, entry]) => {
    ...
    const hasCapitalLetters = word !== word.toLowerCase();

    return startsWithArticle && hasCapitalLetters;
  });

  console.log(
    `📊 Found ${capitalizedArticlePhrases.length} capitalized article phrases`
  );

  // Extract word information
  const phrasesList = capitalizedArticlePhrases.map(([id, entry]) => ({
    id: entry.id,
    word: entry.word,
...
export const summary = {
  totalPhrases: ${
    phrasesList.length
  }, // Capitalized article phrases extracted for review
  breakdown: {
...
`;
  ...
  console.log(
    "✅ Updated phrases-target-list.js with capitalized article phrases"
  );
  console.log(`📊 Total capitalized article phrases: ${phrasesList.length}`);
  console.log(`📁 Output file: phrases-target-list.js`);

  // Show some statistics
  console.log("\n📈 CAPITALIZED ARTICLE PHRASES BREAKDOWN:");
  ...
  console.error(
    "❌ Error extracting capitalized article phrases:",
    error.message
  );
  process.exit(1);
}
