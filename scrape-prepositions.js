...
/**
 * Cambridge Dictionary Scraper for Prepositions
 * Scrapes all prepositions with proper part of speech matching
 */
      ...
      for (let i = 0; i < headEntries.length; i++) {
        ...
        if (posElement) {
          ...
          // 5. If one is matching, then scrape it (more flexible matching)
            ...
            expectedPartOfSpeech === "preposition" &&
            (posText.includes("preposition") ||
              posText.includes("prep") ||
              posText.includes("adverb") ||
              posText.includes("conjunction"))
          ) {
            console.log(
              `    ✅ Found flexible preposition match in POS at index ${i + 1}`
            );
            foundMatch = true;
          }
        }
...
async function scrapeAllPrepositions() {
  console.log(
    `\n🚀 Starting Cambridge scraping for ${prepositions.size} prepositions...`
  );

  const scrapedPrepositions = new Map();
  const failedPrepositions = [];

  let count = 0;
  for (const [id, prepositionData] of prepositions) {
    count++;
    console.log(
      `\n📝 Processing ${count}/${prepositions.size}: ${prepositionData.word} (${prepositionData.partOfSpeech})`
    );

    try {
      const scrapedData = await scrapeCambridgeWord(
        prepositionData.word,
        prepositionData.partOfSpeech
      );

      if (scrapedData) {
        scrapedPrepositions.set(id, scrapedData);
        console.log(`✅ Successfully scraped ${prepositionData.word}`);
      } else {
        failedPrepositions.push({
          word: prepositionData.word,
          reason: "No data returned",
        });
        console.log(
          `❌ Failed to scrape ${prepositionData.word} - no data returned`
        );
      }
    } catch (error) {
      failedPrepositions.push({
      ...
      console.log(
        `❌ Failed to scrape ${prepositionData.word} - ${error.message}`
      );
    }

    // Wait random 3-5 seconds between requests
    if (count < prepositions.size) {
  ...
  const outputData = {
    ...
    total_prepositions: prepositions.size,
    successful_scrapes: scrapedPrepositions.size,
    failed_scrapes: failedPrepositions.length,
    prepositions: Object.fromEntries(scrapedPrepositions),
    failed_prepositions: failedPrepositions,
  };

  const outputPath = "src/data/dictionary/words/cambridge/prepositions.js";
  const outputContent = `/**
 * Cambridge Dictionary Scraped Prepositions
 * Generated: ${new Date().toISOString()}
 * Successfully scraped: ${scrapedPrepositions.size}/${prepositions.size}
 */

export const prepositionsCambridge = new Map([
${Array.from(scrapedPrepositions.entries())
...
export const scrapeStats = {
  ...
  total_prepositions: ${outputData.total_prepositions},
  successful_scrapes: ${outputData.successful_scrapes},
  failed_scrapes: ${outputData.failed_scrapes},
  failed_prepositions: ${JSON.stringify(
    outputData.failed_prepositions,
    null,
    2
  )}
};
`;
  ...
  console.log(`\n🎉 Scraping complete!`);
  console.log(`📊 Results:`);
  console.log(`  - Total prepositions: ${prepositions.size}`);
  console.log(`  - Successfully scraped: ${scrapedPrepositions.size}`);
  console.log(`  - Failed: ${failedPrepositions.length}`);
  console.log(`  - Saved to: ${outputPath}`);

  if (failedPrepositions.length > 0) {
    console.log(`\n❌ Failed prepositions:`);
    failedPrepositions.forEach((failed) => {
...
// Run the scraper
scrapeAllPrepositions().catch(console.error);
