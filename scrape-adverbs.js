...
/**
 * Cambridge Dictionary Scraper for Adverbs
 * Scrapes all adverbs with proper part of speech matching
 */
...
async function scrapeAllAdverbs() {
  console.log(
    `\n🚀 Starting Cambridge scraping for ${adverbs.size} adverbs...`
  );
  ...
  let count = 0;
  for (const [id, adverbData] of adverbs) {
    count++;
    console.log(
      `\n📝 Processing ${count}/${adverbs.size}: ${adverbData.word} (${adverbData.partOfSpeech})`
    );

    try {
      const scrapedData = await scrapeCambridgeWord(
        adverbData.word,
        adverbData.partOfSpeech
      );

      if (scrapedData) {
        scrapedAdverbs.set(id, scrapedData);
        console.log(`✅ Successfully scraped ${adverbData.word}`);
      } else {
        failedAdverbs.push({
          word: adverbData.word,
          reason: "No data returned",
        });
        console.log(
          `❌ Failed to scrape ${adverbData.word} - no data returned`
        );
      }
    } catch (error) {
      failedAdverbs.push({
      ...
      console.log(`❌ Failed to scrape ${adverbData.word} - ${error.message}`);
    }

    // Wait random 4-6 seconds between requests
    if (count < adverbs.size) {
  ...
  const outputData = {
    ...
    total_adverbs: adverbs.size,
    successful_scrapes: scrapedAdverbs.size,
    failed_scrapes: failedAdverbs.length,
    adverbs: Object.fromEntries(scrapedAdverbs),
    failed_adverbs: failedAdverbs,
  };

  const outputPath = "src/data/dictionary/words/cambridge/adverbs.js";
  const outputContent = `/**
 * Cambridge Dictionary Scraped Adverbs
 * Generated: ${new Date().toISOString()}
 * Successfully scraped: ${scrapedAdverbs.size}/${adverbs.size}
 */
...
export const scrapeStats = {
  ...
  total_adverbs: ${outputData.total_adverbs},
  successful_scrapes: ${outputData.successful_scrapes},
  failed_scrapes: ${outputData.failed_scrapes},
  failed_adverbs: ${JSON.stringify(outputData.failed_adverbs, null, 2)}
};
`;
  ...
  console.log(`\n🎉 Scraping complete!`);
  console.log(`📊 Results:`);
  console.log(`  - Total adverbs: ${adverbs.size}`);
  console.log(`  - Successfully scraped: ${scrapedAdverbs.size}`);
  console.log(`  - Failed: ${failedAdverbs.length}`);
  console.log(`  - Saved to: ${outputPath}`);

  if (failedAdverbs.length > 0) {
    console.log(`\n❌ Failed adverbs:`);
    failedAdverbs.forEach((failed) => {
...
// Run the scraper
scrapeAllAdverbs().catch(console.error);
