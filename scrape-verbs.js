...
/**
 * Cambridge Dictionary Scraper for Verbs
 * Scrapes all verbs with proper part of speech matching
 */
      ...
      for (let i = 0; i < headEntries.length; i++) {
        ...
        if (posElement) {
          ...
          // Flexible matching for verbs
            ...
            console.log(
              `    ✅ Found flexible verb match in POS at index ${i + 1}`
            );
            foundMatch = true;
          }
        }
...
async function scrapeAllVerbs() {
  console.log(`\n🚀 Starting Cambridge scraping for ${verbs.size} verbs...`);

  const scrapedVerbs = new Map();
  const failedVerbs = [];

  let count = 0;
  for (const [id, verbData] of verbs) {
    count++;
    console.log(
      `\n📝 Processing ${count}/${verbs.size}: ${verbData.word} (${verbData.partOfSpeech})`
    );
    ...
    try {
      ...
      if (scrapedData) {
        scrapedVerbs.set(id, scrapedData);
        console.log(`✅ Successfully scraped ${verbData.word}`);
      } else {
        failedVerbs.push({
          word: verbData.word,
          reason: "No data returned",
        });
        console.log(`❌ Failed to scrape ${verbData.word} - no data returned`);
      }
    } catch (error) {
      failedVerbs.push({
        word: verbData.word,
      ...
      console.log(`❌ Failed to scrape ${verbData.word} - ${error.message}`);
    }
  ...
  const outputData = {
    ...
    total_verbs: verbs.size,
    successful_scrapes: scrapedVerbs.size,
    failed_scrapes: failedVerbs.length,
    verbs: Object.fromEntries(scrapedVerbs),
    failed_verbs: failedVerbs,
  };

  const outputPath = "src/data/dictionary/words/cambridge/verbs.js";
  const outputContent = `/**
 * Cambridge Dictionary Scraped Verbs
 * Generated: ${new Date().toISOString()}
 * Successfully scraped: ${scrapedVerbs.size}/${verbs.size}
 */

export const verbsCambridge = new Map([
${Array.from(scrapedVerbs.entries())
...
export const scrapeStats = {
  ...
  total_verbs: ${outputData.total_verbs},
  successful_scrapes: ${outputData.successful_scrapes},
  failed_scrapes: ${outputData.failed_scrapes},
  failed_verbs: ${JSON.stringify(outputData.failed_verbs, null, 2)}
};
`;
  ...
  console.log(`\n🎉 Scraping complete!`);
  console.log(`📊 Results:`);
  console.log(`  - Total verbs: ${verbs.size}`);
  console.log(`  - Successfully scraped: ${scrapedVerbs.size}`);
  console.log(`  - Failed: ${failedVerbs.length}`);
  console.log(`  - Saved to: ${outputPath}`);

  if (failedVerbs.length > 0) {
    console.log(`\n❌ Failed verbs:`);
    failedVerbs.forEach((failed) => {
...
// Run the scraper
scrapeAllVerbs().catch(console.error);
