...
export const useSectionTime = (moduleId, sectionId, isActive = true) => {
  ...
  const updateSectionTime = useCallback(
    async (additionalSeconds) => {
      if (!supabaseUser || !moduleId || !sectionId || additionalSeconds <= 0)
        return;

      try {
        // Get current section progress
        const { data: currentProgress, error: fetchError } =
        ...
        logger.analytics(
          `Updated section time: ${moduleId}/${sectionId} +${additionalSeconds}s (total: ${newTotalTime}s)`
        );
      } catch (error) {
  ...
  const startTracking = useCallback(() => {
    ...
    logger.analytics(
      `Started tracking section time for: ${moduleId}/${sectionId}`
    );
  ...
  const stopTracking = useCallback(() => {
    ...
    if (!isIdle() && timeSpent > 0) {
      ...
      logger.analytics(
        `Stopped tracking section time for: ${moduleId}/${sectionId}, time: ${timeSpent}s`
      );
    }
