...
/**
 * Cambridge Dictionary Scraper for Conjunctions
 * Scrapes all conjunctions with proper part of speech matching
 */
      ...
      for (let i = 0; i < headEntries.length; i++) {
        ...
        if (posElement) {
          ...
          // 5. If one is matching, then scrape it (more flexible matching)
            ...
            expectedPartOfSpeech === "conjunction" &&
            (posText.includes("conjunction") ||
              posText.includes("conj") ||
              posText.includes("connector"))
          ) {
            console.log(
              `    ✅ Found flexible conjunction match in POS at index ${i + 1}`
            );
            foundMatch = true;
          }
        }
...
async function scrapeAllConjunctions() {
  console.log(
    `\n🚀 Starting Cambridge scraping for ${conjunctions.size} conjunctions...`
  );

  const scrapedConjunctions = new Map();
  const failedConjunctions = [];

  let count = 0;
  for (const [id, conjunctionData] of conjunctions) {
    count++;
    console.log(
      `\n📝 Processing ${count}/${conjunctions.size}: ${conjunctionData.word} (${conjunctionData.partOfSpeech})`
    );

    try {
      const scrapedData = await scrapeCambridgeWord(
        conjunctionData.word,
        conjunctionData.partOfSpeech
      );

      if (scrapedData) {
        scrapedConjunctions.set(id, scrapedData);
        console.log(`✅ Successfully scraped ${conjunctionData.word}`);
      } else {
        failedConjunctions.push({
          word: conjunctionData.word,
          reason: "No data returned",
        });
        console.log(
          `❌ Failed to scrape ${conjunctionData.word} - no data returned`
        );
      }
    } catch (error) {
      failedConjunctions.push({
      ...
      console.log(
        `❌ Failed to scrape ${conjunctionData.word} - ${error.message}`
      );
    }

    // Wait random 3-5 seconds between requests
    if (count < conjunctions.size) {
  ...
  const outputData = {
    ...
    total_conjunctions: conjunctions.size,
    successful_scrapes: scrapedConjunctions.size,
    failed_scrapes: failedConjunctions.length,
    conjunctions: Object.fromEntries(scrapedConjunctions),
    failed_conjunctions: failedConjunctions,
  };

  const outputPath = "src/data/dictionary/words/cambridge/conjunctions.js";
  const outputContent = `/**
 * Cambridge Dictionary Scraped Conjunctions
 * Generated: ${new Date().toISOString()}
 * Successfully scraped: ${scrapedConjunctions.size}/${conjunctions.size}
 */

export const conjunctionsCambridge = new Map([
${Array.from(scrapedConjunctions.entries())
...
export const scrapeStats = {
  ...
  total_conjunctions: ${outputData.total_conjunctions},
  successful_scrapes: ${outputData.successful_scrapes},
  failed_scrapes: ${outputData.failed_scrapes},
  failed_conjunctions: ${JSON.stringify(
    outputData.failed_conjunctions,
    null,
    2
  )}
};
`;
  ...
  console.log(`\n🎉 Scraping complete!`);
  console.log(`📊 Results:`);
  console.log(`  - Total conjunctions: ${conjunctions.size}`);
  console.log(`  - Successfully scraped: ${scrapedConjunctions.size}`);
  console.log(`  - Failed: ${failedConjunctions.length}`);
  console.log(`  - Saved to: ${outputPath}`);

  if (failedConjunctions.length > 0) {
    console.log(`\n❌ Failed conjunctions:`);
    failedConjunctions.forEach((failed) => {
...
// Run the scraper
scrapeAllConjunctions().catch(console.error);
