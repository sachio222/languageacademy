...
/**
 * Cambridge Dictionary Scraper for Failed Adjectives
 * Re-scrapes the adjectives that previously failed
 */

// Failed adjectives from the previous run
const failedAdjectives = [
  "historiques",
  "rapides",
  "autres",
  "nouvelle",
...
// Helper function to wait for a random time between 4-6 seconds
...
async function scrapeCambridgeWord(word, expectedPartOfSpeech = null) {
  ...
  // 2. For plural forms: detect and map to singular
  const pluralEndings = ["s", "es", "aux", "eux"];
  for (const ending of pluralEndings) {
    if (actualWord.endsWith(ending) && actualWord.length > ending.length + 2) {
      const singular = actualWord.slice(0, -ending.length);
      if (singular.length > 2) {
        // Avoid very short words
        actualWord = singular;
        isPlural = true;
        console.log(`  - Detected plural form, using singular: ${actualWord}`);
        break;
      }
    }
  }
      ...
      for (let i = 0; i < headEntries.length; i++) {
        ...
        if (posElement) {
          ...
          // 5. If one is matching, then scrape it (more flexible matching)
            ...
            (posText.includes("adjective") ||
              posText.includes("adj") ||
              posText.includes("qualifier"))
          ) {
            console.log(
              `    ✅ Found flexible adjective match in POS at index ${i + 1}`
            );
            foundMatch = true;
          }
        }
...
async function scrapeFailedAdjectives() {
  console.log(
    `\n🚀 Re-scraping ${failedAdjectives.length} failed adjectives with improved logic...`
  );

  const scrapedAdjectives = new Map();
  const stillFailedAdjectives = [];

  let count = 0;
  for (const word of failedAdjectives) {
    count++;
    console.log(
      `\n📝 Processing ${count}/${failedAdjectives.length}: ${word} (adjective)`
    );

    try {
      const scrapedData = await scrapeCambridgeWord(word, "adjective");

      if (scrapedData) {
        scrapedAdjectives.set(`${word}-fr`, scrapedData);
        console.log(`✅ Successfully scraped ${word}`);
      } else {
        stillFailedAdjectives.push({
          word: word,
          reason: "No data returned",
        });
        console.log(`❌ Failed to scrape ${word} - no data returned`);
      }
    } catch (error) {
      stillFailedAdjectives.push({
        word: word,
      ...
      console.log(`❌ Failed to scrape ${word} - ${error.message}`);
    }

    // Wait random 4-6 seconds between requests
    if (count < failedAdjectives.length) {
  ...
  // Save results
  const outputPath =
    "src/data/dictionary/words/cambridge/failed-adjectives-retry.js";
  const outputContent = `/**
 * Cambridge Dictionary Scraped Failed Adjectives (Retry)
 * Generated: ${new Date().toISOString()}
 * Successfully scraped: ${scrapedAdjectives.size}/${failedAdjectives.length}
 */

export const failedAdjectivesRetry = new Map([
${Array.from(scrapedAdjectives.entries())
...
export const retryStats = {
  ...
  total_failed_adjectives: ${failedAdjectives.length},
  successfully_scraped: ${scrapedAdjectives.size},
  still_failed: ${stillFailedAdjectives.length},
  still_failed_adjectives: ${JSON.stringify(stillFailedAdjectives, null, 2)}
};
`;
  ...
  console.log(`📊 Results:`);
  console.log(`  - Total failed adjectives: ${failedAdjectives.length}`);
  console.log(`  - Successfully scraped: ${scrapedAdjectives.size}`);
  console.log(`  - Still failed: ${stillFailedAdjectives.length}`);
  console.log(`  - Saved to: ${outputPath}`);

  if (stillFailedAdjectives.length > 0) {
    console.log(`\n❌ Still failed adjectives:`);
    stillFailedAdjectives.forEach((failed) => {
...
// Run the scraper
scrapeFailedAdjectives().catch(console.error);
