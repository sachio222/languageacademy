...
/**
 * Cambridge Dictionary Scraper for Articles
 * Scrapes all articles with proper part of speech matching
 */
    ...
    if (expectedPartOfSpeech) {
      ...
      for (let i = 0; i < headEntries.length; i++) {
        ...
        if (posElement) {
          ...
          // Check for regex patterns for articles
          else if (expectedPartOfSpeech === "article") {
            const articlePatterns = [
              /determiner/i,
              /indefinite\s+article/i,
              /definite\s+article/i,
              /article/i,
            ];

            for (const pattern of articlePatterns) {
              if (pattern.test(posText)) {
                console.log(
                  `    ✅ Found regex match in POS at index ${
                    i + 1
                  } (${pattern})`
                );
                foundMatch = true;
...
async function scrapeAllArticles() {
  console.log(
    `\n🚀 Starting Cambridge scraping for ${articles.size} articles...`
  );

  const scrapedArticles = new Map();
  const failedArticles = [];

  let count = 0;
  for (const [id, articleData] of articles) {
    count++;
    console.log(
      `\n📝 Processing ${count}/${articles.size}: ${articleData.word} (${articleData.partOfSpeech})`
    );

    try {
      const scrapedData = await scrapeCambridgeWord(
        articleData.word,
        articleData.partOfSpeech
      );

      if (scrapedData) {
        scrapedArticles.set(id, scrapedData);
        console.log(`✅ Successfully scraped ${articleData.word}`);
      } else {
        failedArticles.push({
        ...
        console.log(
          `❌ Failed to scrape ${articleData.word} - no data returned`
        );
      }
    } catch (error) {
      failedArticles.push({ word: articleData.word, reason: error.message });
      console.log(`❌ Failed to scrape ${articleData.word} - ${error.message}`);
    }
  ...
  const outputData = {
    ...
    successful_scrapes: scrapedArticles.size,
    failed_scrapes: failedArticles.length,
    articles: Object.fromEntries(scrapedArticles),
    failed_articles: failedArticles,
  };

  const outputPath = "articles-cambridge.js";
  const outputContent = `/**
 * Cambridge Dictionary Scraped Articles
 * Generated: ${new Date().toISOString()}
 * Successfully scraped: ${scrapedArticles.size}/${articles.size}
 */

export const articlesCambridge = new Map([
${Array.from(scrapedArticles.entries())
...
export const scrapeStats = {
  ...
  total_articles: ${outputData.total_articles},
  successful_scrapes: ${outputData.successful_scrapes},
  failed_scrapes: ${outputData.failed_scrapes},
  failed_articles: ${JSON.stringify(outputData.failed_articles, null, 2)}
};
`;
  ...
  console.log(`  - Successfully scraped: ${scrapedArticles.size}`);
