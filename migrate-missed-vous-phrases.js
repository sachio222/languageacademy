/**
 * Migrate Missed Vous Verb Phrases
 * Move the 4 remaining "vous" verb phrases to verb-phrases.js
 */
...
// The 4 missed vous phrases
const missedVousPhrases = [
  "vous croyez",
  "vous parlez",
  "vous prenez",
  "vous savez",
...
console.log("🚀 Migrating 4 missed vous verb phrases...");
...
try {
  ...
  // Process each missed vous phrase
  const newVerbPhrases = [];
  const updatedVerbPhrases = [];

  for (const phrase of missedVousPhrases) {
    try {
  ...
  console.log("✅ Updated verb-phrases.js with missed vous phrases");
  ...
  if (allPhrasesMatch) {
    const currentPhrases = allPhrasesMatch[1]
      ...
      .filter((phrase) => !missedVousPhrases.includes(phrase));
    ...
    console.log(
      `✅ Removed ${missedVousPhrases.length} missed vous phrases from phrases-word-list.js`
    );
  }

  // Summary
  console.log("\n🎉 MIGRATION COMPLETE!");
  console.log(`✅ Total processed: ${totalProcessed} missed vous phrases`);
  ...
  console.log("\n📁 Files updated:");
  console.log(
    "  - src/data/dictionary/phrases/verb-phrases.js (missed vous phrases added/updated)"
  );
