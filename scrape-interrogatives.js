...
/**
 * Cambridge Dictionary Scraper for Interrogatives
 * Scrapes all interrogatives with proper part of speech matching
 */
      ...
      for (let i = 0; i < headEntries.length; i++) {
        ...
        if (posElement) {
          ...
          // 5. If one is matching, then scrape it (more flexible matching)
            ...
            expectedPartOfSpeech === "interrogative" &&
            (posText.includes("interrogative") ||
              posText.includes("question") ||
              posText.includes("pronoun") ||
              posText.includes("adverb"))
          ) {
            console.log(
              `    ✅ Found flexible interrogative match in POS at index ${
                i + 1
              }`
            );
            foundMatch = true;
...
async function scrapeAllInterrogatives() {
  console.log(
    `\n🚀 Starting Cambridge scraping for ${interrogatives.size} interrogatives...`
  );

  const scrapedInterrogatives = new Map();
  const failedInterrogatives = [];

  let count = 0;
  for (const [id, interrogativeData] of interrogatives) {
    count++;
    console.log(
      `\n📝 Processing ${count}/${interrogatives.size}: ${interrogativeData.word} (${interrogativeData.partOfSpeech})`
    );

    try {
      const scrapedData = await scrapeCambridgeWord(
        interrogativeData.word,
        interrogativeData.partOfSpeech
      );

      if (scrapedData) {
        scrapedInterrogatives.set(id, scrapedData);
        console.log(`✅ Successfully scraped ${interrogativeData.word}`);
      } else {
        failedInterrogatives.push({
          word: interrogativeData.word,
          reason: "No data returned",
        });
        console.log(
          `❌ Failed to scrape ${interrogativeData.word} - no data returned`
        );
      }
    } catch (error) {
      failedInterrogatives.push({
      ...
      console.log(
        `❌ Failed to scrape ${interrogativeData.word} - ${error.message}`
      );
    }

    // Wait random 3-5 seconds between requests
    if (count < interrogatives.size) {
  ...
  const outputData = {
    ...
    total_interrogatives: interrogatives.size,
    successful_scrapes: scrapedInterrogatives.size,
    failed_scrapes: failedInterrogatives.length,
    interrogatives: Object.fromEntries(scrapedInterrogatives),
    failed_interrogatives: failedInterrogatives,
  };

  const outputPath = "src/data/dictionary/words/cambridge/interrogatives.js";
  const outputContent = `/**
 * Cambridge Dictionary Scraped Interrogatives
 * Generated: ${new Date().toISOString()}
 * Successfully scraped: ${scrapedInterrogatives.size}/${interrogatives.size}
 */

export const interrogativesCambridge = new Map([
${Array.from(scrapedInterrogatives.entries())
...
export const scrapeStats = {
  ...
  total_interrogatives: ${outputData.total_interrogatives},
  successful_scrapes: ${outputData.successful_scrapes},
  failed_scrapes: ${outputData.failed_scrapes},
  failed_interrogatives: ${JSON.stringify(
    outputData.failed_interrogatives,
    null,
    2
  )}
};
`;
  ...
  console.log(`\n🎉 Scraping complete!`);
  console.log(`📊 Results:`);
  console.log(`  - Total interrogatives: ${interrogatives.size}`);
  console.log(`  - Successfully scraped: ${scrapedInterrogatives.size}`);
  console.log(`  - Failed: ${failedInterrogatives.length}`);
  console.log(`  - Saved to: ${outputPath}`);

  if (failedInterrogatives.length > 0) {
    console.log(`\n❌ Failed interrogatives:`);
    failedInterrogatives.forEach((failed) => {
...
// Run the scraper
scrapeAllInterrogatives().catch(console.error);
