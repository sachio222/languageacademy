...
/**
 * Cambridge Dictionary Scraper for Adjectives
 * Scrapes all adjectives with proper part of speech matching
 */
...
async function scrapeAllAdjectives() {
  console.log(
    `\n🚀 Starting Cambridge scraping for ${adjectives.size} adjectives...`
  );
  ...
  let count = 0;
  for (const [id, adjectiveData] of adjectives) {
    count++;
    console.log(
      `\n📝 Processing ${count}/${adjectives.size}: ${adjectiveData.word} (${adjectiveData.partOfSpeech})`
    );

    try {
      const scrapedData = await scrapeCambridgeWord(
        adjectiveData.word,
        adjectiveData.partOfSpeech
      );

      if (scrapedData) {
        scrapedAdjectives.set(id, scrapedData);
        console.log(`✅ Successfully scraped ${adjectiveData.word}`);
      } else {
        failedAdjectives.push({
          word: adjectiveData.word,
          reason: "No data returned",
        });
        console.log(
          `❌ Failed to scrape ${adjectiveData.word} - no data returned`
        );
      }
    } catch (error) {
      failedAdjectives.push({
      ...
      console.log(
        `❌ Failed to scrape ${adjectiveData.word} - ${error.message}`
      );
    }
  ...
  const outputData = {
    ...
    total_adjectives: adjectives.size,
    successful_scrapes: scrapedAdjectives.size,
    failed_scrapes: failedAdjectives.length,
    adjectives: Object.fromEntries(scrapedAdjectives),
    failed_adjectives: failedAdjectives,
  };

  const outputPath = "src/data/dictionary/words/cambridge/adjectives.js";
  const outputContent = `/**
 * Cambridge Dictionary Scraped Adjectives
 * Generated: ${new Date().toISOString()}
 * Successfully scraped: ${scrapedAdjectives.size}/${adjectives.size}
 */
...
export const scrapeStats = {
  ...
  total_adjectives: ${outputData.total_adjectives},
  successful_scrapes: ${outputData.successful_scrapes},
  failed_scrapes: ${outputData.failed_scrapes},
  failed_adjectives: ${JSON.stringify(outputData.failed_adjectives, null, 2)}
};
`;
  ...
  console.log(`\n🎉 Scraping complete!`);
  console.log(`📊 Results:`);
  console.log(`  - Total adjectives: ${adjectives.size}`);
  console.log(`  - Successfully scraped: ${scrapedAdjectives.size}`);
  console.log(`  - Failed: ${failedAdjectives.length}`);
  console.log(`  - Saved to: ${outputPath}`);

  if (failedAdjectives.length > 0) {
    console.log(`\n❌ Failed adjectives:`);
    failedAdjectives.forEach((failed) => {
...
// Run the scraper
scrapeAllAdjectives().catch(console.error);
