/**
 * Migrate Classified Phrases to Proper Files
 * Move all classified phrases from phrases-target-list.js to their respective category files
 * Remove them from phrases.js, phrases-target-list.js, and phrases-word-list.js
 */
...
// Import the target list to get the classified phrases
import {
...
console.log("🚀 Migrating classified phrases to their proper files...");
...
try {
  ...
  if (prepositionalPhrases.length > 0) {
        ...
        if (originalData) {
          const newEntry = [
            `${phrase.replace(/\s+/g, "-").replace(/[^a-zA-Z0-9-]/g, "")}-fr`,
            {
              ...
              tags: [
                ...(originalData.entry.tags || []),
                "prepositional-phrase",
              ],
            },
          ];
          prepositionalEntries.push(newEntry);
    ...
    if (prepositionalEntries.length > 0) {
      categoryMigrations.push({
        category: "prepositional-phrases",
        entries: prepositionalEntries,
        count: prepositionalEntries.length,
      });
    }
  }
  ...
  if (greetings.length > 0) {
    console.log(`\n📝 Processing ${greetings.length} greetings...`);
    ...
    for (const phrase of expressions) {
      try {
        ...
        if (originalData) {
          ...
          expressionEntries.push(newEntry);
          console.log(`✅ Processed: "${phrase}"`);
          totalProcessed++;
        } else {
          console.log(`❌ Phrase "${phrase}" not found in original phrases`);
    ...
    for (const phrase of verbPhrases) {
      try {
        ...
        if (originalData) {
          ...
          verbPhraseEntries.push(newEntry);
          console.log(`✅ Processed: "${phrase}"`);
          totalProcessed++;
        } else {
          console.log(`❌ Phrase "${phrase}" not found in original phrases`);
  ...
  if (timeExpressions.length > 0) {
    console.log(
      `\n📝 Processing ${timeExpressions.length} time expressions...`
    );
    ...
    for (const phrase of timeExpressions) {
      try {
        ...
        if (originalData) {
          ...
          timeExpressionEntries.push(newEntry);
          console.log(`✅ Processed: "${phrase}"`);
          totalProcessed++;
        } else {
          console.log(`❌ Phrase "${phrase}" not found in original phrases`);
  ...
  if (locationPhrases.length > 0) {
    console.log(
      `\n📝 Processing ${locationPhrases.length} location phrases...`
    );
    ...
    for (const phrase of locationPhrases) {
      try {
        ...
        if (originalData) {
          ...
          locationPhraseEntries.push(newEntry);
          console.log(`✅ Processed: "${phrase}"`);
          totalProcessed++;
        } else {
          console.log(`❌ Phrase "${phrase}" not found in original phrases`);
    ...
    for (const phrase of articlePhrases) {
      try {
        ...
        if (originalData) {
          ...
          articlePhraseEntries.push(newEntry);
          console.log(`✅ Processed: "${phrase}"`);
          totalProcessed++;
        } else {
          console.log(`❌ Phrase "${phrase}" not found in original phrases`);
  ...
  if (properNouns.length > 0) {
    console.log(`\n📝 Processing ${properNouns.length} proper nouns...`);
    ...
    for (const phrase of properNouns) {
      try {
        ...
        if (originalData) {
          ...
          properNounEntries.push(newEntry);
          console.log(`✅ Processed: "${phrase}"`);
          totalProcessed++;
        } else {
          console.log(`❌ Phrase "${phrase}" not found in original phrases`);
  ...
  // 9. CONJUNCTION PHRASES
  if (conjunctionPhrases.length > 0) {
    console.log(
      `\n📝 Processing ${conjunctionPhrases.length} conjunction phrases...`
    );
    const conjunctionPhraseEntries = [];

    for (const phrase of conjunctionPhrases) {
      try {
        const originalData = phrasesLookup.get(phrase);
        if (originalData) {
          const newEntry = [
            `${phrase.replace(/\s+/g, "-").replace(/[^a-zA-Z0-9-]/g, "")}-fr`,
            {
              ...
              tags: [...(originalData.entry.tags || []), "conjunction-phrase"],
            },
          ];
          conjunctionPhraseEntries.push(newEntry);
          console.log(`✅ Processed: "${phrase}"`);
          totalProcessed++;
        } else {
          console.log(`❌ Phrase "${phrase}" not found in original phrases`);
    ...
    if (conjunctionPhraseEntries.length > 0) {
      categoryMigrations.push({
        category: "conjunctions",
        entries: conjunctionPhraseEntries,
        count: conjunctionPhraseEntries.length,
      });
    }
  }
    ...
    for (const phrase of adjectivePhrases) {
      try {
        ...
        if (originalData) {
          ...
          adjectivePhraseEntries.push(newEntry);
          console.log(`✅ Processed: "${phrase}"`);
          totalProcessed++;
        } else {
          console.log(`❌ Phrase "${phrase}" not found in original phrases`);
...
`;
    ...
    console.log(
      `✅ Updated ${migration.category}.js with ${migration.count} new entries`
    );
  }
 ...
 * UPDATED: All classified phrases migrated to their respective category files, arrays cleared
 */
